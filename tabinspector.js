// Copyright (c) 2018 Mark S. Thew. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

/**
 * Retrieve all windows along with their tabs and pass them to the provided callback
 * @param {*} callback 
 */
function getTabsByWindow (callback) {
  chrome.windows.getAll({
    populate: true // include tab details
  }, (windows) => {
    callback(windows)
  })
}

/**
 * Get the current URL.
 *
 * @param {function(string)} callback called when the URL of the current tab
 *   is found.
 */
function getTabs (callback) {
  // Query filter to be passed to chrome.tabs.query - see
  // https://developer.chrome.com/extensions/tabs#method-query
  var queryInfo = {
//  active: false,
    currentWindow: true
  }

  chrome.tabs.query(queryInfo, (tabs) => {
    // chrome.tabs.query invokes the callback with a list of tabs that match the
    // query. When the popup is opened, there is certainly a window and at least
    // one tab, so we can safely assume that |tabs| is a non-empty array.
    // A window can only have one active tab at a time, so the array consists of
    // exactly one tab.
//  var tab = tabs[0]

    // A tab is a plain object that provides information about the tab.
    // See https://developer.chrome.com/extensions/tabs#type-Tab
//  var url = tab.url

    // tab.url is only available if the "activeTab" permission is declared.
    // If you want to see the URL of other tabs (e.g. after removing active:true
    // from |queryInfo|), then the "tabs" permission is required to see their
    // "url" properties.
//  console.assert(typeof url === 'string', 'tab.url should be a string')

    callback(tabs)
  })

  // Most methods of the Chrome extension APIs are asynchronous. This means that
  // you CANNOT do something like this:
  //
  // var url;
  // chrome.tabs.query(queryInfo, (tabs) => {
  //   url = tabs[0].url;
  // });
  // alert(url); // Shows "undefined", because chrome.tabs.query is async.
}

var loadTabs1 = function (tabs) {
  var tree = createTree('tab-tree', 'white')
  var openTabNode = tree.createNode('Open Tabs 1', true, 'images/star.png', null, null, null)
  var suspendTabNode = tree.createNode('Suspended Tabs 1', true, 'images/star.png', null, null, null)

  // Load the URLs into the tabs list as a <ul> elements.
  for (var i = 0; i < tabs.length; i++) {
    var tab = tabs[i]
    if (tab.active) {
      openTabNode.createChildNode(tab.title, false, 'images/monitor.png', null, null)
    } else {
      openTabNode.createChildNode(tab.title, false, 'images/blue_key.png', null, null)
    }
  }

  tree.drawTree()
}

var loadTabs2 = function (windows) {
  var tree = createTree('tab-tree', 'white')
  for (var i = 0; i < windows.length; i++) {
    var window = windows[i]
    var windowNode = tree.createNode(
      'Window ' + i,
      true,
      (window.focused) ? 'images/monitor.png' : 'images/folder.png',
      null,
      null,
      null)
    for (var t = 0; t < window.tabs.length; t++) {
      var tab = window.tabs[t]
      windowNode.createChildNode(
        tab.title,
        false,
        (tab.active) ? 'images/star.png' : 'images/blue_key.png',
        null,
        null)
    }
  }
  tree.drawTree()
}

/**
 * This extension loads the open tabs and displays them in a tree structure.
 */
document.addEventListener('DOMContentLoaded', () => {
//  getTabs(loadTabs1)
  getTabsByWindow(loadTabs2)
})
